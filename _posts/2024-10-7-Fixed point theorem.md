...
var('x')
@interact
def cobwebber(f_text = input_box(default = "cos(x)",label = "function", type=str), start_val = slider(0,1,.01,.5,label = 'start value'), its = slider([i+1 for i in range(0,12)],default = 16, label="iterations")):
    def f(x):
        return eval(f_text)
